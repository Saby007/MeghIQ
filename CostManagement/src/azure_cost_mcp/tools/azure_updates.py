...
def _parse_date(date_str: str) -> str | None:
    ...
    if not date_str:
        return None
    # RFC 822 / RSS date format: "Mon, 23 Feb 2026 18:15:17 Z"
    formats = [
        "%a, %d %b %Y %H:%M:%S %Z",
        "%a, %d %b %Y %H:%M:%S %z",
        "%Y-%m-%dT%H:%M:%SZ",
    ...
    # Handle trailing " Z" (space before Z)
...
def _detect_status(title: str) -> str:
    ...
    Reads whatever the feed provides in square brackets or known patterns.
    Examples:
        "[Launched] ..." → "Launched"
        ...
        "Update: ..." → "Update"
    """
    # Check for bracketed prefix pattern: [Status Text]
    bracket_match = re.match(r"^\[(.+?)\]\s*", title)
    if bracket_match:
        return bracket_match.group(1).strip()

    # Check for colon-prefixed patterns
...
def _clean_title(title: str) -> str:
    ...
    # Remove bracketed prefix
    cleaned = re.sub(r"^\[.+?\]\s*", "", title)
    # Remove colon prefix patterns
...
def normalize_category_tokens(category: str) -> set[str]:
    ...
    # Remove "azure" prefix — it's ubiquitous and adds no value
    normalised = re.sub(r"^azure\s+", "", normalised)

    # Extract parenthetical aliases: "(AKS)" → "aks"
    aliases: list[str] = []
...
def _parse_feed_item(item: ET.Element) -> dict[str, Any]:
    ...
    # Separate meta-categories (lifecycle/classification tags) from
    # service categories. Meta-categories describe the *nature* of the
    # update; service categories describe *what service* it's about.
    # We detect meta-categories dynamically: they are categories that
    # don't reference a specific Azure service.
    meta_categories: list[str] = []
    service_categories: list[str] = []
    for cat in categories:
        cat_lower = cat.lower()
        # These are classification/lifecycle tags the feed uses
        # We detect them by checking if they appear as status-like tags
        # or are generic classification terms
        if cat_lower in {
    ...
    # Build category tokens from service categories for matching
    all_category_tokens: set[str] = set()
    for cat in service_categories:
        all_category_tokens |= normalize_category_tokens(cat)

    # Also tokenize meta-categories separately
    ...
    # Tokenize the clean title for matching
    title_tokens = set(re.findall(r"[a-z0-9]+", clean_title.lower()))
    # Remove very common noise from title tokens
    ...
    return {
        ...
        # Tokens for matching (not included in final output, used internally)
...
async def list_all_azure_updates(
    ...
    Args:
        category: Filter by service category (e.g., "Compute", "Networking").
                  Case-insensitive partial match.
        status: Filter by status (e.g., "Launched", "In preview", "Retirement").
                Case-insensitive partial match.
    ...
    Returns:
        JSON array of Azure update items with title, status, categories,
...
async def get_azure_update_details(
    ...
    Args:
        update_id: The update ID (numeric GUID from the feed).

    Returns:
        JSON with full update details including all categories and description.
    """
    try:
        updates = await _fetch_and_parse_feed()
        ...
        if match is None:
            return error_response(
                ...
                "The feed may only contain recent updates.",
                code="NOT_FOUND",
            )
