...
async def _execute_query(scope: str, payload: dict[str, Any]) -> str:
    ...
    async with httpx.AsyncClient(timeout=60.0) as client:
        ...
        if resp.status_code != 200:
            body = resp.text
            return error_response(
                f"Azure API error {resp.status_code}: {body}",
...
async def query_subscription_costs(
    ...
    Args:
        ...
        group_by: Comma-separated dimension names to group by.
                  Common: ServiceName, ResourceGroup, ResourceType, MeterCategory,
                  ResourceLocation, ChargeType, PublisherType, TagKey:YourTag.
...
async def compare_costs(
    ...
    try:
        ...
        if resp1.status_code != 200:
            return error_response(f"Period 1 query failed: {resp1.text}")
        if resp2.status_code != 200:
            return error_response(f"Period 2 query failed: {resp2.text}")
