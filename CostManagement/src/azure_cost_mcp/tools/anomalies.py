...
Note: The Anomaly API may require specific API versions and may not be available
in all regions. Falls back to a cost-spike detection heuristic if the native API
is unavailable.
"""
...
async def list_anomalies(
    ...
    This tool first attempts to use the Azure Cost Anomaly Detection API.
    If that API is not available, it falls back to a heuristic that identifies
    daily cost spikes exceeding 2 standard deviations from the rolling average.

    Args:
        subscription_id: Azure subscription ID (defaults to AZURE_SUBSCRIPTION_ID env var).
        days_back: Number of days to look back for anomalies (default 30).

    Returns:
        JSON array of detected anomalies with date, expected cost, actual cost,
        and deviation percentage.
    """
    try:
        sub_id = subscription_id or get_subscription_id()

        # --- Attempt 1: Native Anomaly Detection API ---
        result = await _try_native_anomaly_api(sub_id)
        if result is not None:
            return result

        # --- Fallback: Heuristic spike detection from daily cost data ---
...
async def _try_native_anomaly_api(subscription_id: str) -> str | None:
    ...
    # The anomaly detection endpoint is at a preview API version
    api_version = "2023-11-01"
    scope = f"subscriptions/{subscription_id}"
    url = (
        f"{BASE_URL}/{scope}/providers/Microsoft.CostManagement/"
        f"anomalies?api-version={api_version}"
    )
    token = get_token()
    ...
    try:
        ...
        # If 404 or not supported, fall through to None
        if resp.status_code in (404, 400, 501):
            return None

        # Other errors — report them
        return error_response(
            f"Anomaly API error {resp.status_code}: {resp.text}",
...
async def _heuristic_anomaly_detection(
    subscription_id: str,
    days_back: int,
) -> str:
    ...
    """
    from azure_cost_mcp.tools.cost_query import _build_query_payload, _parse_query_response
    ...
    if resp.status_code != 200:
        return error_response(
            f"Cost query for anomaly detection failed {resp.status_code}: {resp.text}"
        )
    ...
    for row in rows:
        ...
        # The date column name varies; try common names
    ...
    if len(daily_costs) < 8:
        return success_response(
            ...
            extra_meta={
                "source": "heuristic",
                "message": "Not enough data points for anomaly detection (need at least 8 days).",
            },
        )
