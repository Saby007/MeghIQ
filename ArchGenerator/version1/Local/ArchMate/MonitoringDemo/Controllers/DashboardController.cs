...
namespace MonitoringDemo.Controllers
{
    ...
    public class DashboardController : ControllerBase
    {
        ...
        {
            var htmlPath = Path.Combine(Directory.GetCurrentDirectory(), "wwwroot", "dashboard.html");
            try
            {
            ...
            {
                _logger.LogError(ex, "Failed to load dashboard HTML file from path: {Path}", htmlPath);
                return "<html><body><h1>Dashboard temporarily unavailable</h1><p>Error: " + ex.Message + "</p></body></html>";
            }
        }
    }
}