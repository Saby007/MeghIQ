...
async def _execute_query(scope: str, payload: dict[str, Any]) -> str:
    ...
    async with httpx.AsyncClient(timeout=60.0) as client:
        ...
        if resp.status_code != 200:
            return error_response(
                sanitize_error_message(f"Azure API error {resp.status_code}: {resp.text}"),
...
async def query_management_group_costs(
    ...
    try:
        management_group_id = validate_management_group_id(management_group_id)
...
async def compare_costs(
    ...
    try:
        ...
        if resp1.status_code != 200:
            return error_response(sanitize_error_message(f"Period 1 query failed: {resp1.text}"))
        if resp2.status_code != 200:
            return error_response(sanitize_error_message(f"Period 2 query failed: {resp2.text}"))
