...
async def _try_native_anomaly_api(subscription_id: str) -> str | None:
    ...
    try:
        ...
        return error_response(sanitize_error_message(f"Anomaly API error {resp.status_code}: {resp.text}"), code=str(resp.status_code))
...
async def _heuristic_anomaly_detection(subscription_id: str, days_back: int) -> str:
    ...
    if resp.status_code != 200:
        return error_response(sanitize_error_message(f"Cost query for anomaly detection failed {resp.status_code}: {resp.text}"))
