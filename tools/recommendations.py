        ...
        async with httpx.AsyncClient(timeout=60.0) as client:
            while url:
                ...
                if resp.status_code != 200:
                    return error_response(sanitize_error_message(f"Azure Advisor API error {resp.status_code}: {resp.text}"), code=str(resp.status_code))
    ...
    try:
        ...
        if recommendation_id.startswith("/") or recommendation_id.startswith("subscriptions/"):
            ...
            full_id = validate_azure_resource_id(
                f"subscriptions/{sub_id}/providers/Microsoft.Advisor/recommendations/{recommendation_id}"
            )
        ...
        if resp.status_code != 200:
            return error_response(sanitize_error_message(f"Advisor API error {resp.status_code}: {resp.text}"), code=str(resp.status_code))
